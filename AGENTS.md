...
4. AI-extract a **512×512** `profile.jpg` with the `extract-profile` skill (GenerateImage isolation onto white).
...
Snapshot vs profile: `product-detail-snapshot.jpg` is the raw capture. `profile.jpg` is an AI extract onto white (`extract-profile` skill). `profile_extract` in `listing.md` is the per-SKU prompt addendum. Commit `profile.jpg`, `profile.meta.yaml`, and catalog changes together.
